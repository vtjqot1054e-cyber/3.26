# 数字真我·第三批精炼入库（9条，实际8条通过+1条B-4降级不入库）
# 来源：数字真我-11复盘报告（复盘大脑独立分析）
# 审核人：数字真我-12，2026-03-23
# B-精炼-4（SPE文件匹配方式）降级为技术备忘，不入向量库

import json
import os
from vector_db import get_client, get_collection, add_refined, count

DATA_FILE = os.path.join(os.path.dirname(__file__), "..", "batch_data", "batch_003.json")
